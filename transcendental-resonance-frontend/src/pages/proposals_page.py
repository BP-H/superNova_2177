...
async def proposals_page():
    ...
    with (
        ...
        p_type = ui.select(
            ["general", "system_parameter_change"], value="general"
        ).classes("w-full mb-2")
        ...
        ui.button("Create Proposal", on_click=create_proposal).classes(
            "w-full mb-4"
                    ...
                    with (
                        ...
                        if p["status"] == "open":
                            ...
                            async def vote_yes(p_id=p["id"]):
                                api_call(
                                    "POST", f"/proposals/{p_id}/vote", {"vote": "yes"}
                                )
                                await refresh_proposals()

                            async def vote_no(p_id=p["id"]):
                                api_call(
                                    "POST", f"/proposals/{p_id}/vote", {"vote": "no"}
                                )
                                await refresh_proposals()

                            ui.row().classes("justify-between")
                            ui.button("Yes", on_click=vote_yes).style(
                                "background: green; color: white;"
                            )
                            ui.button("No", on_click=vote_no).style(
