...
async def groups_page():
    ...
    with page_container(THEME):
        ...
        ui.label("Recommended").classes("text-xl mb-2").style(
            f'color: {THEME["accent"]};'
        )
        recommended_list = ui.column().classes("w-full mb-4")

        async def refresh_recommended() -> None:
            recs = await api_call("GET", "/groups/recommended") or []
            ...
            for r in recs:
                with recommended_list:
                    with (
                        ...
                        ui.label(r["name"]).classes("text-lg")
                        if r.get("description"):
                            ui.label(r["description"]).classes("text-sm")

                        async def join_fn(g_id=r["id"]):
                            await api_call("POST", f"/groups/{g_id}/join")
                            await refresh_recommended()
                            await refresh_groups()

                        ui.button("Join", on_click=join_fn).style(
        ...
        async def refresh_groups():
            ...
            if search_query.value:
                groups = [
                    g for g in groups if search_query.value.lower() in g["name"].lower()
                ]
            if sort_select.value:
