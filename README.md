...
register themselves with `register_route(name, func, description=None, category=None)` and are invoked through
`dispatch_route`.

A convenient read-only route `"list_routes"` returns the currently available
route names. The `"help"` route provides a structured listing grouped by
category. This can help debug which backend callbacks are present:
