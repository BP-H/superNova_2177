...
def certify_validations_comprehensive(
   ...
   if enable_full_analysis:
       try:
           ...
           consensus_scores = {"default_hypothesis": avg_score}  # Simplified for single hypothesis
           ...
           if risk_level == "high":
               if certification in ["strong", "provisional"]:
                   certification = "experimental"
                   recommendations.append("Certification downgraded due to high integrity risk")
           elif risk_level == "medium":
               if certification == "strong":
                   certification = "provisional"
                   recommendations.append("Certification downgraded due to medium integrity risk")
           
           # Generate specific recommendations
           if diversity_result.get("diversity_score", 0) < 0.3:
               recommendations.append("Increase validator diversity across specialties and affiliations")
           
           if coordination_result.get("overall_risk_score", 0) > 0.5:
               recommendations.append("Investigate potential validator coordination patterns")
           
           if temporal_result.get("consensus_volatility", 0) > 0.4:
               recommendations.append("Review temporal consistency of validation submissions")
